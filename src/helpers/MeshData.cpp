...
int MeshData::load_obj(std::string fullfilename, float scale, bool optimize)
{
	...
	gl.vao.SetAttributeBufferSource(gl.texcoordbuffer.m_sizeInBytes ? gl.texcoordbuffer : gl.normalbuffer, 2, 2 , GL_FLOAT, GL_FALSE);
...
{
	if (!materialListBuffer.m_uiBufferIdx)
	{
